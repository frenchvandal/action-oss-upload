...
(async () => {
  try {
    ...
    for await (const file of localFiles) {
      ...
      await exec(
        `aliyun Cdn RefreshObjectCaches --ObjectPath https://frenchvandal.cn/${toPosixPath(relative(homeDir, file))} --ObjectType file`,
      );
      await exec(
        `aliyun Cdn PushObjectCache --ObjectPath https://frenchvandal.cn/${toPosixPath(relative(homeDir, file))} --Area overseas --L2Preload true`,
      );
    }
